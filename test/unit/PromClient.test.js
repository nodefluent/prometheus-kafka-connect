...
describe("PromClient Unit", function() {
    ...
    describe("Errornous value", function() {
        ...
        it("should fail when object has been registered", function(done) {
          ...
          try {
            const metric = promclient._getMetricObject({metric: "fail_1"});
        ...
        it("should fail on non string metric", function(done) {
          ...
          try {
            const metric = promclient._checkThrowable({metric: 123, value: 456});
          } catch (err) {
            const message = err.message;
            assert.deepEqual(message, expected);
            done();
        ...
        it("should fail on non number value", function(done) {
          ...
          try {
            const metric = promclient._checkThrowable({metric: "fail_2", value: "foo"});
          } catch (err) {
            const message = err.message;
            assert.deepEqual(message, expected);
            done();
        ...
        it("should fail on non string type", function(done) {
          ...
          try {
            const metric = promclient._checkThrowable({metric: "fail_3", value: 1, type: 123});
          } catch (err) {
            const message = err.message;
            assert.deepEqual(message, expected);
            done();
        ...
        it("should fail on non string help", function(done) {
          ...
          try {
            const metric = promclient._checkThrowable({metric: "fail_4", value: 1, help: 123});
          } catch (err) {
            const message = err.message;
            assert.deepEqual(message, expected);
            done();
